...
async fn edit_product(
    ...
    let product_sku = path.into_inner();
    let new_sku = body.sku.unwrap_or(product_sku as u32);
    let query_result = sqlx::query_as!(
        Product,
        "SELECT * FROM products WHERE sku = $1",
        product_sku as i32
    ...
    let query_result = sqlx::query_as!(
        Product,
        "UPDATE products SET title = $1, description = $2, sku = $3, category = $4, quantity = $5, price = $6, sale_price = $7, on_sale = $8 WHERE sku = $9 RETURNING *",
        body.title.to_owned().unwrap_or(product.title),
        body.description.to_owned().unwrap_or(product.description),
        body.sku.to_owned().unwrap_or(product.sku.try_into().unwrap()) as i32,
